...
class CustomTTAReporter implements Reporter {
    ...
    onTestEnd(test: TestCase, result: TestResult): void {
        ...
        const consoleLogs = this.associateLogsWithSteps(test, result, currentTestSteps);
    ...
    private extractTestLogs(result: TestResult): string[] {
        ...
        for (const chunk of result.stderr) {
            const text = typeof chunk === 'string' ? chunk : chunk.toString();
            allLogs.push(...text.split('\n').filter((line: string) => line.trim()).map((line: string) => `[stderr] ${line}`));
        }
    ...
    private associateLogsWithSteps(_test: TestCase, result: TestResult, testSteps: StepData[]): string[] {
        const allLogs = this.extractTestLogs(result);

        if (allLogs.length === 0 || testSteps.length === 0) {
            return allLogs;
        }
...
<body><p>Redirecting to <a href="${path.basename(this.outputFile)}">latest report</a>...</p></body></html>`;
    ...
    private generateHistoryPage(reportDir: string): void {
        const files = fs.readdirSync(reportDir)
            .filter((f: string) => f.startsWith('report_') && f.endsWith('.html'))
            .sort()
            .reverse();
...
<body>
    ...
    private generateTestDetailPanel(test: TestData): string {
        ...
        if (test.consoleLogs && test.consoleLogs.length > 0) {
            ...
            <div class="detail-section console-section">
                <div class="section-header" onclick="toggleSection(this)">
                    <span class="section-arrow">▼</span> Console Output (${test.consoleLogs.length} lines)
                </div>
                <div class="section-content">
    ...
    private getStyles(): string {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .console-section .section-header { cursor: pointer; }
